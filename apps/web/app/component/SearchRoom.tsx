      ...
      timeOut.current=setTimeout(()=>fetchRoomDetail(searchParam),delay);
		...
		const joinRoom = async (roomId:string) => {
            ...
            if(!roomId){
               ...
               console.log(joinRoomResponse.data.data.room.name,"roomName in the joinroom");
               ...
               setRoomDetail([])
				...
				if (error instanceof Error) {
					console.log(
						error,
						"error in form of joining the room",
						error.message
					);
				} else {
			...
			<div className="space-y-4">
				...
				<div className="overflow-y-scroll h-60 scroll-smooth border-1 rounded-lg">
					{roomDetail.length>0
						...
						: <p className="text-2xl flex items-center justify-center h-full">No Such Room</p>}
				</div>
			</div>
		);
}
